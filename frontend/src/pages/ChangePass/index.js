...
const ChangePass = () => {
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    await AuthServices.requestVerify(
      storedUserDataString._id,
      storedUserDataString.email,
    );
    setIsLoading(false);
  ...
  const handleVerify = async (OTP) => {
    try {
      const response = await AuthServices.checkVerify(
        storedUserDataString._id,
        OTP,
      );
      if (!response.data.isAuthenticated) return;
      setIsLoading(true);
      await changePass(storedUserDataString._id, {
        pass: pass,
        newPass: newPass,
      });
      setPass("");
      ...
      <DefaultLayout>
        ...
        {isOpenEmailVerification && (
          <EmailVerification
            ...
            email={storedUserDataString.email}
